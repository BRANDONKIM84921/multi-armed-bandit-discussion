...
4. What if we altered what $\epsilon$ throughout the whole process (i.e. $\epsilon = 0.25$ for the first 10 rolls, and $\epsilon = 0.05$ for the last 10 rolls). What are some pros and cons to this alteration? (This strategy is known as **Decaying Epsilon Greedy**)
...
$$ a(t) = \hat{\mu_i} + \sqrt{\frac{2\ln{t}}{n_i}}$$

Where
- $\hat{\mu}$ is the estimate of the mean of the arm $i$
- $\n_i$ is the number of times we rolled from arm $i$
...
For this situation, let's limit our scope from optimizing our yield to instead be just making a profit (any profit would be good). This way, we could use a beta-binomial conjugate. 
